...
from rodeo.kalmantv import square_root, standard
...
def _solve_filter(
    ...
    **params
    ...
    def scan_fun(carry, filter_kwargs):
        ...
        # output
        carry = {"state_filt": (mean_state_next, var_state_next)}
        stack = {
    ...
    # scan initial value
    scan_init = {"state_filt": (mean_state_init, var_state_init)}
    if key is not None:
        keys = jax.random.split(key, num=n_steps)
    else:
        keys = jnp.zeros(n_steps)

    filter_kwargs = {"t": jnp.arange(n_steps), "key": keys}
    # scan itself
    ...
    **params
    ...
    def scan_fun(x_state_next, smooth_kwargs):
        ...
        x_state_curr = jax.random.multivariate_normal(
            key, mean_state_sim, var_state_sim, method="svd"
        )
    ...
    x_state_smooth = jnp.concatenate([ode_init[None], scan_out, scan_init[None]])
...
def solve_mv(
    ...
    **params
    ...
    def scan_fun(state_next, smooth_kwargs):
        ...
        state_curr = {"mean": mean_state_curr, "var": var_state_curr}
        return state_curr, state_curr

    # initialize
    scan_init = {"mean": mean_state_filt[n_steps], "var": var_state_filt[n_steps]}
    # scan arguments
    scan_kwargs = {
        "mean_state_filt": mean_state_filt[1:n_steps],
        "var_state_filt": var_state_filt[1:n_steps],
    ...
    var_state_smooth = jnp.concatenate(
        [
            jnp.zeros((n_block, n_bstate, n_bstate))[None],
            scan_out["var"],
            scan_init["var"][None],
        ]
    )
