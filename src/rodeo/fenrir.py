...
def backward_param(mean_state_filt, var_state_filt, 
    ...
    Returns:
        (tuple):
        - **wgt_state_cond** (ndarray(n_steps, n_block, n_bstate, n_bstate)): Transition of smooth conditional at time t given observations from times [0...T]; :math:`A_{n|N}`.
...
def backward(n_res, wgt_state, mean_state, var_state,
    ...
    Args:
        ...
        wgt_state (ndarray(n_steps, n_block, n_bstate, n_bstate)): Transition matrix defining the solution prior; :math:`A_{0:N}`.
        ...
        wgt_obs (ndarray(n_obs, n_block, n_bobs, n_state)): Transition matrix defining the noisy observations; :math:`D_{0:M}`.
    ...
    n_obs, n_block, n_bobs, n_bstate = wgt_obs.shape
...
def fenrir(key, fun, W, x0, theta, tmin, tmax, n_res,
    ...
    Args:
        ...
        wgt_state (ndarray(n_block, n_bstate, n_bstate)): Transition matrix defining the solution prior; :math:`Q`.
        var_state (ndarray(n_block, n_bstate, n_bstate)): Variance matrix defining the solution prior; :math:`R`.
        wgt_obs (ndarray(n_block, n_bobs, n_state)): Transition matrix defining the noisy observations; :math:`D`.
        var_obs (ndarray(n_block, n_bobs, n_bobs)): Variance matrix defining the noisy observations; :math:`\Omega`.
        y_obs (ndarray(n_steps, n_block, n_bobs)): Observed data; :math:`y_{0:M}`.
        interrogate (function): Function defining the linearization method.
    ...
    # reverse pass
    logdens, _ = backward(
        n_res=n_res, wgt_state=wgt_state_cond, 
        mean_state=mean_state_cond, var_state=var_state_cond, 
        wgt_obs=wgt_obs, var_obs=var_obs, y_obs=y_obs
    )
...
def fenrir_mv(key, fun, W, x0, theta, tmin, tmax, n_res,
              wgt_state, var_state,
              wgt_obs, var_obs, y_obs, interrogate):
