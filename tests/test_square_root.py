...
class TestKalmanTVGM(unittest.TestCase):
    ...
    def test_predict(self):
        mean_state_past, var_state_past, mean_state_pred1, var_state_pred1, _, _ = (
            utils.test_filter(self)
        )
        ...
        comparison_check(
            self, mean_state_pred1, mean_state_pred2, var_state_pred1, var_state_pred2
        )

    def test_update(self):
        _, _, mean_state_pred, var_state_pred, mean_state_filt1, var_state_filt1 = (
            utils.test_filter(self)
        )
        ...
        comparison_check(
            self, mean_state_filt1, mean_state_filt2, var_state_filt1, var_state_filt2
        )
    ...
    def test_smooth_mv(self):
        ...
        comparison_check(
            self,
            mean_state_smooth1[0],
            mean_state_smooth2,
            var_state_smooth1[0, :, 0, :].squeeze(),
            var_state_smooth2,
        )
    ...
    def test_smooth_sim(self):
        ...
        comparison_check(
            self, mean_state_sim1, mean_state_sim2, V, var_state_sim2, places=2
        )
