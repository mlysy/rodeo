...
def theta_plot(theta_double, theta_fenrir, theta_diffrax, theta_true, step_sizes, var_names, clip=None, rows=1):
    ...
    plot_theta = []
    theta = []
    if theta_double is not None:
        plot_theta.append('pope')
        theta.append(theta_double)
...
def theta_plotwd(theta_double, theta_fenrir, theta_true, step_sizes, var_names, clip=None, rows=1):
    r"""Plot the distribution of :math:`\theta` using the Kalman solver 
        and the Euler approximation."""
    n_hlst, _, n_theta = theta_double.shape
    ncol = ceil(n_theta/rows) +1
    ...
    if theta_fenrir is not None:
        ...
        theta.append(theta_fenrir)
    nrow = len(plot_theta)
    fig = plt.figure(figsize=(25, 10*rows))
