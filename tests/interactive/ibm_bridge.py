...
from rodeo.kalmantv import _solveV
...
def bridge_prior(A, Q, R, Y, n_res, n_block):
    ...
    Returns:
        (dict):
        - **trans_state** (ndarray(n_eval, n_block, p, p)) Transition matrix defining the solution prior; :math:`Q_n`.
        - **mean_state** (ndarray(n_eval, n_block, p)): Transition_offsets defining the solution prior; denoted by :math:`c_n`.
        - **var_state** (ndarray(n_eval, n_block, p, p)) Variance matrix defining the solution prior; :math:`R_n`.
    """
    n_y = Y.shape[0]
    mean_state = jax.vmap(lambda t:
                jax.vmap(lambda r: 
                jax.vmap(lambda b: jnp.matmul(A[r, b], Y[t, r, b]))
                        (jnp.arange(n_block)))(jnp.arange(n_res)))(jnp.arange(n_y))
    mean_state = jnp.reshape(mean_state, newshape=(n_y*n_res, n_block, -1))
    trans_state = jnp.tile(Q.T, n_y).T
    var_state = jnp.tile(R.T, n_y).T
    
    init = {"trans_state": trans_state, "mean_state": mean_state, "var_state": var_state}
    return init
...
def ibm_bridge_init(n_res, dt, n_deriv, sigma, Y, W, Omega):
    r"""
    ...
    """
    n_block = len(n_deriv)
    init = jax.vmap(lambda r: ibm_init(r*dt, n_deriv, sigma))(jnp.arange(max(2, n_res)))
    # Q1 = init['trans_state'][1]
