...
# np.save("saves/theta_diffrax.npy", theta_diffrax)
# np.save("saves/theta_fenrirg.npy", theta_fenrir)
# np.save("saves/theta_doubleg.npy", theta_double)
# theta_diffrax = np.load("saves/theta_diffraxg.npy")
# theta_fenrir = np.load("saves/theta_fenrir.npy")
# # theta_double = np.load("saves/theta_double.npy")

plt.rcParams.update({'font.size': 20})
var_names = ['a', 'b', 'c', r"$V(0)$", r"$R(0)$"]
param_true = np.append(theta_true, np.array([-1, 1]))
hlist = [1/4, 1/10]
figure = theta_plot(theta_double, theta_fenrir, theta_diffrax, param_true, hlist, var_names, clip=[None, (0, 0.5), None, None, None], rows=1)
