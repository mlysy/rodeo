...
import tests.gauss_markov as gm
...
def fitz_setup(self):
    ...
    self.x0_block = jnp.array([[-1.0, 1, 0], [1.0, 1 / 3, 0]])
...
def kalman_setup(self):
    ...
    self.wgt_state = 0.01 * random.normal(
        subkeys[2], (self.n_tot - 1, self.n_state, self.n_state)
    )
    # wgt_state = jnp.zeros((n_tot-1, self.n_state, self.n_state))
    self.mean_meas = random.normal(
        subkeys[3],
        (
            self.n_tot,
            self.n_meas,
