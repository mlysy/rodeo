...
# Chkrbetii ----------------------------------------------------------------------------------------------------------
def chkrebtii(X, t, **params):
    "Chkrebtii ODE."
    return jnp.array([[jnp.sin(2*t) - X[0, 0]]])

def chkrebtii_nb(X, t, **params):
...
W_block = jnp.zeros((n_vars, 1, n_deriv))  # LHS matrix of ODE
W_block = W_block.at[:, :, 1].set(1)
W = jsl.block_diag(*W_block)
x0_block = jnp.array([[-1., 1., 0.], [1., 1/3, 0.]])  # initial value for the IVP
x0 = x0_block.flatten()
...
x0_block = hes1_init(jnp.array(ode0), theta)  # initial value for the IVP
x0 = x0_block.flatten()
...
W_block = W_block.at[:, :, 1].set(1)
W = jsl.block_diag(*W_block)
theta = jnp.array([2.23, 0.034, 0.55, 5.1, 2.3, 0.36])
thetanp = np.array(theta)

ode0 = np.array([63804435., 15492., 21752., 0., 618013., 93583.])
x0_block = seirah_init(jnp.array(ode0), theta)  # initial value for the IVP
x0 = x0_block.flatten()
